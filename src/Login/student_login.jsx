    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (statusResponse.status === 200) {
          ...
          if (studentSemester === statusSemester) {
            navigate(`/enrollment/subjects/${rollNumber}`);
          } else {
            navigate(`/enrollment-not-started?rollNumber=${rollNumber}&semester=${statusResponse.data}`);
          }
        } else if (statusResponse.status === 204) {
          setError('No enrollment status available at this time.');
        }
      }
    } catch (error) {
      if (error.response) {
  ...
  return (
    <div className="wrapper">
      <div className="navbar">
        <div className="logo">
          <h1 style={{ color: '#fff', fontSize: '24px', margin: '0 20px' }}>Student Portal</h1>
        </div>
      </div>
